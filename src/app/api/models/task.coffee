...
.factory("Task", (resourcePlus, DoubtfireConstants, $http) ->
  ...
  Task.generateLatestAssessmentUrl = (task) ->
    "#{DoubtfireConstants.API_URL}/projects/#{task.project().project_id}/task_def_id/#{task.definition.id}/submissions/latest"

  Task.getSubmissionByTimestampUrl = (task, timestamp) ->
    "#{DoubtfireConstants.API_URL}/projects/#{task.project().project_id}/task_def_id/#{task.definition.id}/submissions/timestamps/#{timestamp}"
